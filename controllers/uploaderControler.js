...
module.exports = {
  uploadFile: (req, res) => {
    try {
      let basePath = '/image';
      let { caption, media, filename, iduser } = req.body;
      ...
      // });
      let sqlInsert = `Insert into album values(null, ${db.escape(caption)},${db.escape(filepath)}, '${moment().format('YYYY-MM-DD HH:mm:ss')}', ${db.escape(iduser)})`;
      db.query(sqlInsert, (err, results) => {
        ...
        res.status(200).send({ message: 'upload file berhasil' });
      });
    } catch (error) {
      console.log(error);
      res.status(500).send(error);
